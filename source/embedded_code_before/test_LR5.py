...
image_ok = multiprocessing.Value('b', False)
...
def capture_and_preprocess(image_lock, image_ok, running):
    global imagednn, image, image2
    while running.value:
        ...
        with image_lock:
            ...
            if ret2:
                image2 = img2
            image_ok.value = ret1 and ret2

def opencvdnn_thread(image_lock, image_ok, running):
    global imagednn
...
if __name__ == '__main__':
    capture_process = multiprocessing.Process(target=capture_and_preprocess, args=(image_lock, image_ok, running))
    dnn_process = multiprocessing.Process(target=opencvdnn_thread, args=(image_lock, image_ok, running))
